...
const SidebarLeft = ({ onButtonClick }) => {
    ...
    let userList = useSelector((state) => state.handleChat.chatUsers)
    ...
    useEffect(() => {
        ...
        socket.on('chatReadSuccess', (data) => {
            ...
            //     })
            // }
            // console.log('chat read bar left', data)
            if (data.length != 0) {
                data.forEach((chat) => {
                    chat.is_read = 1;
                });
    ...
    //         if (u._id === id) {
    //             u.is_online = 0; // Setting is_online to 0 for matched user
    //             return u; // Return updated or unchanged user object
    //         }
    //     })
    //     onButtonClick(user)
    ...
    //     let user = users.map(u => {
    //         if (u._id === id) {
    //             u.is_online = 1; // Setting is_online to 0 for matched user
    //             return u; // Return updated or unchanged user object
    //         }
    //     })
    //     onButtonClick(user)
            ...
            <div className="w-full h-[113px]">
                <div className="w-full py-2.5 px-4 profile-bar flex justify-between items-center bg-[#f0f2f5]">
                    <div className="profile-pic w-[40px] h-[40px] rounded-full overflow-hidden">
                        <img style={{ height: '100%' }} src={loginUser?.profile?.profile_pic} className='w-100 h-100 object-cover' alt="" />
                    </div>
