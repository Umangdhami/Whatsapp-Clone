...
const Rightside = ({ data, reciver }) => {
    ...
    //             setVisibleChats(prev => [...new Set([...prev, messageId])]);
    //         } else {
    //             setVisibleChats(prev => prev.filter(id => id !== messageId));
    //         }
    //     });
    // }, []);
    ...
    useEffect(() => {
        ...
        //     if (senderId == data.sender_id && data.reciver_id == reciverId) {
        //         setChats(chats.map((chat) => chat._id == data._id ? { ...chat, is_recived: 1 } : chat))
        //         console.log(chats, 'lk');
        //     }
        // });
        // ************************
    ...
    const deleteForMe = async () => {
        ...
        if (res.data.status === true) {
            setChats(chats.filter((item) => item._id !== res.data.data._id));
            socket.emit('delete-chat', res.data.data._id)
    ...
    const deleteForEveryone = async () => {
        ...
        if (res.data.status === true) {
            ...
            setChats(chats.map((item) => item._id === res.data.data._id ? res.data.data : item))
            socket.emit('delete-chat', res.data.data._id, res.data.data)
            setOpenModal(!openModal)
            setChatOptions(false)
        }
    }
    ...
    useEffect(() => {
        ...
        if (chatUser.length !== 0) {
            ...
            localStorage.setItem('senderId', chatUser[0].sender_id)
            localStorage.setItem('reciverId', chatUser[0]._id)
